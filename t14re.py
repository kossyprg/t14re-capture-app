...
class T14re():
    ...
    def connect_radar(self):
        self.COM_port_for_MMIC = self.get_com_port()
        if not self.COM_port_for_MMIC:
            return False
        self.isConnected = True
        
    def load_and_send_config(self):
        path = os.getcwd()
        typ = [('configuration file','*.cfg')]
        path_cfg = filedialog.askopenfilename(filetypes=typ,initialdir=path)
        
        self.config = Config()
        self.config.send_config(path_cfg,self.COM_port_for_MMIC)
        ...
        while not self.isStopped:
            if self.q.qsize() > 1:
                for _ in range(self.q.qsize()):
                    ...
                    f = open(rootFoldername + '/' + foldername + '/' + filename,mode='ab')
    ...
    def capture(self,MaxFiles=0,startDatetime=0,endDatetime=0,useStopBtn=False,rootFoldername='data_t14re'):
        '''
        Start capturing
        ...
        if self.waitUntilGivenDatetime(startDatetime):
            ...
            if MaxFiles<1:
                ...
                if endDatetime==0:
                    if useStopBtn==True:
                        print('Capture until press Stop button')
                        thread1 = threading.Thread(target=self.get_and_put_data)
                        thread2 = threading.Thread(target=self.save_data,args=(rootFoldername,))
                        thread1.start()
                        thread2.start()
                        thread1.join()
                        thread2.join()
    ...
    def PressQuitKeyEvent(self):
        s = 0
        while s!='q' and self.isStopped==False:
